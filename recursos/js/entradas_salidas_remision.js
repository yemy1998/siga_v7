...
function cancel_guia_remision_entradas_salidas(id_ajuste,local_id_params, data_range, io ,moneda_id ){
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      $('#io').val(io);
      $('#io').trigger("chosen:updated");
      

      setTimeout(function(){ 
        $("#historial_list").html('');
        review_info_entradas_salidas(id_ajuste,local_id_params, data_range, io ,moneda_id );
        $('.modal-backdrop').remove(); 
...
function review_info_entradas_salidas(id_ajuste,local_id_params, data_range, io ,moneda_id){
  
  $("#historial_list").html($("#loading").html());
  var accion = '';
  
  $.ajax({
      url: ruta + 'ajuste/historial/filter',
      data: { 
          'local_id': local_id_params,
          'fecha': data_range,
          'moneda_id': moneda_id,
          'io': io 
      },
    ...
    complete:function(xhr,status){
      ...
      if(status=='success'){
        setTimeout(function(){ 
          entradas_salidas_remision_volver(id_ajuste,'entradas_salidas');
        },500)
      }
...
function entradas_salidas_remision_volver(id, tipo){

  $('#dialog_ajuste_remision').html($('#loading').html());
  $('#dialog_ajuste_remision').modal('show');
  ...
  $.ajax({
    ...
    success: function(data) {
      $('#dialog_ajuste_remision').html(data);
    },
    error: function() {
      alert('Error inesperado');
    },
