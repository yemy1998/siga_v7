...
function cancel_guia_remision_traspaso(id_traspaso=null,local_id_params=null, data_range=null){

    $('#dialog_cotizar_confirm').modal('hide');
    $('#loading_save_venta').modal('show');
    var id_de_traspaso = $("#id_traspaso").val(); 
    $.ajax({
      url: ruta + 'traspaso',
      success: function(data) {
        ...
        $('#fecha').val(data_range);

        $('#id-traspaso').val(id_de_traspaso);
     
        $('#locales_chosen').val(local_id_params);
        $('#locales_chosen').trigger("chosen:updated");
        ...
        setTimeout(function(){ 
          ...
          setTimeout(function(){ 
            guia_remision_modal(id_traspaso,'traspaso');
          },600); 
...
/*
function review_info_traspaso(id_venta,local_id, data_range, moneda_id ,document_id,condicion_pago_id){
  
  $("#historial_list").html($("#loading").html());
  var accion = '';
  
  $.ajax({
      url: ruta + 'traspaso/lst_reg_traspasos/'+accion,
      data: { 
          'documento_id':document_id,
          'local_id': local_id,
          'fecha': data_range,
